...
2. 增加信息的可用性，进而提升问题分析的效率：
   + 回调中可以先提供naive异常信息、naive和java调用栈信息和minidump文件文件路径，这些信息可以直接通过业务部门的异常监控系统上报。

   + 业务部门按需拉取minidump文件，其实有了java的调用栈和native的调用栈信息，大部分异常原因都可以快速定位并分析出来，进而提升问题分析的效率。

3. 最少改动：
   + 让接入方不因为引入新功能而大量改动现有功能，例如可以让业务部门继续使用现有的java层异常监控系统，在native崩溃回调处上报native异常信息。
